...
def main():
    ...
    parser.add_argument(
        "--eval_freq", type=int, default=1, help="Only use checkpoints with step " "divisible by eval_freq"
    )
    parser.add_argument("--skip_eval", action="store_true", help="skip evaluate ckpts")
    parser.add_argument(
        "--upload_with_name", default="", type=str, help="if specified, will upload to Azure database " "with this name"
    )
    parser.add_argument(
    ...
    parser.add_argument("--collection", default="grounding", help="the database collection name (grounding)")
    ...
    if args.upload_with_name and is_main_process():
        ...
        while not success:
            try:
                subprocess.run(
                    (
                        "python push_results.py --exp_path={output_dir} " + '--exp_name="{}" ' + "--collection {} {} {}"
                    ).format(
                        args.upload_with_name,
                        args.collection,
                        "--overwrite" if args.overwrite_azure_logs else "",
