...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if dim_text is not None:
            ...
            self.mlp_text = Mlp(
                in_features=dim_text, hidden_features=mlp_hidden_dim_text, act_layer=act_layer, drop=drop
            )
            self.gamma_text = 1.0
            if layer_scale:
    ...
    def __init__(
        ...
        if downsample is not None:
            self.downsample = downsample(
                patch_size=3, in_chans=dim, embed_dim=dim * 2, stride=2, padding=1, norm_layer=norm_layer
            )
        else:
